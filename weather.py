...
## CONFIG
LOCATION_ID = 'ITXX0059'  # Pisa, Italy
## END_CONFIG

# Setup GPIO pin BCM GPIO04
import RPi.GPIO as GPIO
GPIO.setmode( GPIO.BCM )
GPIO.setup( 23, GPIO.IN, pull_up_down=GPIO.PUD_DOWN )	# Next 
...
##		self.tmdateYPosSm = 18		# Time & Date Y Position Small
    ...
    def UpdateWeather( self ):
        ...
        self.w = pywapi.get_weather_from_weather_com(LOCATION_ID)
        w = self.w
        try:
            if ( w[cc]['last_updated'] != self.wLastUpdate ):
                ...
                if ( w[f][0]['high'] == 'N/A' ):
                    ...
                    self.temps[0][0] = w[f][0]['high'] + unichr(0x2103)
                self.temps[0][1] = w[f][0]['low'] + unichr(0x2103)
                self.temps[1][0] = w[f][1]['high'] + unichr(0x2103)
                self.temps[1][1] = w[f][1]['low'] + unichr(0x2103)
                self.temps[2][0] = w[f][2]['high'] + unichr(0x2103)
                self.temps[2][1] = w[f][2]['low'] + unichr(0x2103)
                self.temps[3][0] = w[f][3]['high'] + unichr(0x2103)
                self.temps[3][1] = w[f][3]['low'] + unichr(0x2103)
        except KeyError:
...
#       dtxt = dfont.render( unichr(0x2109), True, lc )
        dtxt = dfont.render(unichr(0x2103), True, lc )
...
GPIO.add_event_detect( 23, GPIO.RISING, callback=btnNext, bouncetime=400)
